app.controller('PDVController', function($scope, $http, $window,$dialogs, UserService) {
	...
	ng.vlr_total_dinheiro  = 0 ;
	ng.vlr_total_outros    = 0 ;
	...
	ng.salvar = function(){
		...
		if(ng.modo_venda == 'est'){
			...
			var troco_opcional = Number($("#troco_opcional").val().replace('.','').replace(',','.'));
			ng.troco 		   = troco_opcional;
			var error_troco    = false ;

			var valor_pagamento_dinheiro = 0 ;
			if(ng.troco > 0){
				$.each(pagamentos,function(key,value){
					if(Number(value.id_forma_pagamento) == 3){
						valor_pagamento_dinheiro = pagamentos[key].valor; 
						if(ng.troco <= valor_pagamento_dinheiro){
			...
			if(error_troco){
				$dialogs.notify('Atenção!','<b>O troco não pode ser maior que a valor recebido em dinheiro</b>');
				btn.button('reset');
				return;				
			}
		...
		if(ng.modo_venda == 'est' && (ng.vlrTotalCompra - ng.total_pg) > 0 ){

			item = {
				id_abertura_caixa		:ng.caixa_aberto.id,
				id_plano_conta   		:ng.caixa.id_plano_caixa,
				id_tipo_movimentacao 	:5,
				valor 					:Math.round((ng.vlrTotalCompra - ng.total_pg) * 100) /100  
			}
		...
		aj.post(baseUrlApi()+"venda",{ produtos: produtos, venda: venda, pagamentos: pagamentos, modo_venda:ng.modo_venda })
			.success(function(data, status, headers, config) {
	...
	ng.aplicarRecebimento = function(){
		...
		if(ng.pagamento.id_forma_pagamento == 3){
			ng.vlr_total_dinheiro += ng.pagamento.valor; 
			...
			ng.vlr_total_outros += ng.pagamento.valor; 
			console.log( ng.pagamento.valor ,ng.vlr_total_outros);
		}
		...
		ng.calculaTroco();
		ng.pagamento = {} ;
		console.log('vlrTotalCompra: '+ng.vlrTotalCompra , 'vlr_total_outros: '+ng.vlr_total_outros,'resto:'+ng.vlrTotalCompra - ng.vlr_total_outros);
	}
