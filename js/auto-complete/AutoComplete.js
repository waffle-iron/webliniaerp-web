					...
					post: function (scope, element, attrs) {
						...
						*/
						...
						scope.select = function(selectedIndex) {
							console.log(scope);
							$('#id_cliente_auto').text(scope.hints_ids[selectedIndex]);
						...
						scope.hints_ids = null;

						var displaySuggestions = function(hintResults) {
							
							scope.hints 	  = hintResults;
							scope.hints_ids   = angular.copy(hintResults);

							$.each(hintResults,function(i,v){
								a = v.split('-');
								hintResults[i] = a[0] ;
							});
